...
export function OwnReview({ gameId = -1, accountId, setOwnReviewCount }){
  const [reviews, setReviews] = useState([]);

  useEffect(() => {
    if(gameId === -1){
      fetch(DB_API + `/Review?ByAccount=${accountId}`)
  ...
  return reviews.length === 0 ? null : (
    <>
      {reviews.map((review) =>
        <div key={review.reviewId} className="listItem reviewBox">
...
export function ReviewsBox({ gameId, accountId, reviewCount }){
  const [reviews, setReviews] = useState([]);
  ...
  return (writeMode ? <WriteReview gameId={gameId} accountId={accountId} setWriteMode={setWriteMode} /> :
    <div className="container">
      <div>
        ...
        <button onClick={() => setWriteMode(true)} disabled={false/*ownReviewCount > 0*/}>Write Review</button>
      </div>
      <div className="listContainer">
        <OwnReview gameId={gameId} accountId={accountId} setOwnReviewCount={setOwnReviewCount} />
        {reviews.map((review) =>
          <div key={review.reviewId} className="listItem reviewBox">
